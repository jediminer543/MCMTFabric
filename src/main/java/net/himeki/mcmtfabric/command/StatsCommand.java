...
public class StatsCommand {
	...
	public static LiteralArgumentBuilder<ServerCommandSource> registerStatus(LiteralArgumentBuilder<ServerCommandSource> root) {
		return root.then(CommandManager.literal("stats").then(CommandManager.literal("reset").executes(cmdCtx -> {
			resetAll();
			return 1;
		})).executes(cmdCtx -> {
			if (!threadStats) {
			...
			cmdCtx.getSource().sendFeedback(message, true);
			return 1;
		}).then(CommandManager.literal("toggle").requires(cmdSrc -> {
			...
			cmdCtx.getSource().sendFeedback(message, true);
			return 1;
		})).then(CommandManager.literal("startlog").requires(cmdSrc -> {
			return cmdSrc.hasPermissionLevel(2);
		}).executes(cmdCtx -> {
			doLogging = true;
			...
			cmdCtx.getSource().sendFeedback(message, true);
			return 1;
		})).then(CommandManager.literal("stoplog").requires(cmdSrc -> {
			return cmdSrc.hasPermissionLevel(2);
		}).executes(cmdCtx -> {
